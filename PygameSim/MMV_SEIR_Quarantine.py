...
if __name__ == '__main__':
    ...
    while True:
        ...
        inf = np.multiply(np.multiply((np.random.rand(n, n) < pInf), dist < dLim),
                          np.multiply(status, contag))  # checks if infected person is within dLim and accout for probabilitstic infectioin
        ...
        for i in range(n):
            ...
            if person.isolate:
                contag[i] = 0
                person.xVel = (QUAR_CENTER[0] - person.x)
                person.yVel = (QUAR_CENTER[1] - person.y)
                vMag = math.sqrt(person.xVel ** 2 + person.yVel ** 2)
                person.xVel *= 0 if vMag == 0 else vTrans / vMag
                person.yVel *= 0 if vMag == 0 else vTrans / vMag
        ...
        quarText = pygame.font.Font('freesansbold.ttf', 32).render("Quarantine " + ("ON" if quar else "OFF"), True, WHITE)
        screen.blit(quarText, (X_LIM-300, 10))
