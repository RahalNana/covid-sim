...
while True:
    ...
    # plt.cla()
    # plt.hist(list(filter(None,nTrans)))
    ...
    if t % Ts == 0:
        epiCurveDat.append((t, nInf, nDead, n))
...
epiCurveDat = np.array(epiCurveDat)
plt.figure("Epicurve")
plt.fill_between(epiCurveDat[:, 0], epiCurveDat[:, 3], color=(0, 0, 1)) # uninfected
# plt.plot(epiCurveDat[:, 0], epiCurveDat[:, 1], color=(1, 0, 0))
plt.fill_between(epiCurveDat[:, 0], epiCurveDat[:, 1] + epiCurveDat[:, 2], color=(0.5, 0.5, 0.5)) # dead
plt.fill_between(epiCurveDat[:, 0], epiCurveDat[:, 1], color=(1, 0, 0)) # infected

plt.figure("No of infections by a person")
plt.hist(list(filter(None, nTrans)), bins=[i+0.5 for i in range(10)])
plt.show()
